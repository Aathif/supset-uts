...
import { smartDateFormatter, getNumberFormatter } from '@superset-ui/core'; // Regexp for the label added to time shifted series
  ...
  setTimeout(() => {
    ...
    svg.selectAll('g.nv-group').filter((d, i) => !stacked || i === countSeriesDisplayed - 1).selectAll('rect').each(function each(d, index) {
      ...
      const textEls = groupLabels.append('text')
        .text(format(stacked ? totalStackedValues[index] : d.y || 0))
        .attr('transform', transformAttr)
        .attr('class', 'bar-chart-label'); // fine tune text position
...
export function generateAreaChartTooltipContent(d, timeFormatter, valueFormatter, chart) {
  ...
  d3.sum(d.series, s => s.value) : // other modes include total row at the end
...
export const sortByMonth = (arr,order='asc') => {
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
  const getIndexOfMonth = month => months.indexOf(month)
  return [ ...arr ].sort((left, right) => {
    const [ leftMonth, leftYear ] = left.x.split('-')
    const [ rightMonth, rightYear ] = right.x.split('-')
    if (leftYear !== rightYear) {
      return order=='asc' ? parseInt(leftYear) - parseInt(rightYear) : parseInt(rightYear) - parseInt(leftYear)
    }
    return order=='asc' ? getIndexOfMonth(leftMonth) - getIndexOfMonth(rightMonth): 
    getIndexOfMonth(rightMonth) - getIndexOfMonth(leftMonth)
  })
}
